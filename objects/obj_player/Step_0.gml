...
// Jump:
if (place_meeting(x, y+1, obj_wall)) && (go_jump) {								// if the player is on a wall object and if jump key is pressed,
	vertical_speed = -5;														// set vertical speed to -5
}
...
if (!place_meeting(x, y+1, obj_wall)) {											// if the player is not on an object
	sprite_index = spr_player_air;												// set player sprite to be "spr_player_air"
	...
	if (horizontal_speed == 0) {												// if the player, is not moving,	
		sprite_index = spr_player;												// set sprite to be "spr_player"
	}
	else {																		// if the player is moving,
		sprite_index = spr_player_run;											// set the player sprite to be "spr_player_run"
	}
}
